...
search_tool/start_date/end_date，导致 agent 侧实际只使用默认的 basic_search_news。
"""
...
class FirstSearchNode(BaseNode):
    ...
    def process_output(self, output: str) -> Dict[str, str]:
        ...
        try:
            ...
            # search_tool/start_date/end_date，故 agent 最终总会回退默认的 basic_search_news。
            return {
...
class ReflectionNode(BaseNode):
    ...
    def process_output(self, output: str) -> Dict[str, str]:
        ...
        try:
            ...
            # 同 FirstSearchNode：只回传 search_query 与 reasoning，丢弃工具/日期等字段。
            return {
                "search_query": search_query,
                "reasoning": reasoning
            }
